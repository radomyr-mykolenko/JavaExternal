...
@WebServlet(name = "CreateCarsTableServlet")
public class CreateCarsTableServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            queryUpdate.execute(SqlQueries.CREATE_CARS_TABLE);
            connectDB.stop();
            response.setContentType("text/html");
            PrintWriter out = response.getWriter();
            out.println("<h3>Table Cars created</h3>");
